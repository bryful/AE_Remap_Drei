...
namespace AE_RemapExceed
{
	public class TSGrid : Control
	{
		...
		public TSGrid()
		{
			...
			funcs.setFunc(funcCmd.ScriptToClip, ScriptToClip);
			funcs.setFunc(funcCmd.ScriptToFile, ScriptToFile);
		...
		public void LayerDataToClipboard()
		{
			...
			try
			{
				ok.Left = this.Parent.Left + this.Left+4+4;
				ok.Top = this.Parent.Top + this.Top + 28;
				ok.Width = this.Width-8;
		...
		protected override void OnSizeChanged(EventArgs e)
		{
			...
			if (tsInfo != null)
			{
				tsInfo.FrameRate = tsd.FrameRate;
				tsInfo.FrameCount = tsd.FrameCountTrue;
				tsInfo.Invalidate();
			}
		}
		//****************************************************************************************
		...
		public void SheetSetting()
		{
			...
			try
			{
				...
				tss.Title = tsd.TITLE;
				tss.SubTitle = tsd.SUB_TITLE;
				tss.OPUS = tsd.OPUS;
				tss.SCECNE = tsd.SCECNE;
				tss.CUT = tsd.CUT;
				tss.Comment = tsd.CommentLines;
				tss.CREATE_USER = tsd.CREATE_USER;
				tss.UPDATE_USER = tsd.UPDATE_USER;
				...
				if (tss.ShowDialog() == DialogResult.OK)
				{
					...
					tsd.TITLE = tss.Title;
					tsd.SUB_TITLE = tss.SubTitle;
					tsd.OPUS = tss.OPUS;
					tsd.SCECNE = tss.SCECNE;
					tsd.CUT = tss.CUT;
					tsd.CommentLines = tss.Comment;
					tsd.CREATE_USER = tss.CREATE_USER;
					tsd.UPDATE_USER = tss.UPDATE_USER;
		...
		public void ScriptToClipLayer( )
		{
			...
			try
			{
				...
				{
					string s = "Layer\"";
					s += tsd.CellCaption(sel.Index) + "\"のCellをScriptにしてCopyしました。";
					ok.execute(150, s);
				}
				else
				{
